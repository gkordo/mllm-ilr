...
def main(args):
    ...
    if args.similarity_file.endswith(".pkl"):
        ...
        print(similarities)
        report = dataset.evaluate(**similarities, k=args.k)
    elif args.similarity_file.endswith(".json"):
        with open(args.similarity_file, "r") as f:
            similarities = json.load(f)
        report = dataset.evaluate(all_similarities=similarities, k=args.k)
    else:
        raise ValueError(
    ...
    print(f"Oracle: {report['oracle']:.2f}%")
