...
from datasets.transforms import InferenceTransforms
...
def get_loaders(queries, similarities, topN, resolution, args):
    image_dict, shards_dict = defaultdict(set), defaultdict(set)
    for q in queries:
        db = sorted(similarities[q], key=lambda x: similarities[q][x], reverse=True)
        for db_img in db[:topN]:
            if ".tar" in db_img:
                shards_dict[db_img].add(q)
            else:
                image_dict[db_img].add(q)

    transform = InferenceTransforms["RESIZE_SIMPLE"].get_transforms(
        resolution=resolution
    )

    query_loader, _ = get_image_dataset(
        args.image_dir,
        preprocess_img=transform,
        num_workers=args.num_workers,
        selected_ids=queries,
    )

    image_loader = None
    if image_dict:
        image_loader, _ = get_image_dataset(
            args.image_dir,
            batch_size=args.batch_size,
            preprocess_img=transform,
            num_workers=args.num_workers,
            selected_ids=list(image_dict.keys()),
        )

    shards_loader = None
    if shards_dict:
        all_tars, all_imgs = zip(*[img.split("/") for img in shards_dict.keys()])
        all_tars = [os.path.join(args.shards_dir, tar) for tar in sorted(set(all_tars))]

        shards_loader, _ = get_wds_dataset(
            all_tars,
            batch_size=args.batch_size,
            preprocess_img=transform,
            num_workers=args.num_workers,
            selected_ids=set(all_imgs),
        )

    return image_dict, shards_dict, query_loader, image_loader, shards_loader
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str]:
        ...
        image_path += ".jpg" if not image_path.endswith(".jpg") else ""
